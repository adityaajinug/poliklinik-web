            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('periksa.create') }}" class="inline-block py-2 px-4 text-white bg-blue-700 rounded-lg hover:bg-blue-800 text-base transform transition duration-150 active:scale-90 focus:scale-95 mb-5">Tambah</a>
                    </div>
                    <div class="overflow-hidden rounded-md">
                        <table class="table-auto w-full rounded-md">
                            ...
                            <tbody>
                                @forelse ($data as $index => $periksa)
                                    <tr class="hover:bg-gray-50">
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $index + 1 }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $periksa->pasien->nama }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $periksa->dokter->nama }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $periksa->tgl_periksa }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $periksa->catatan }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-gray-700">{{ $periksa->obat }}</td>
                                        <td class="border-b border-gray-300 px-4 py-3 text-center">
                                            ...
                                            <form action="{{ route('periksa.destroy', $periksa->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-gray-500 py-4">Tidak ada data pemeriksaan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
